...
export async function getUser(ctx: QueryCtx | MutationCtx, tokenIdentifier: string) {
  const user = await ctx.db
    .query("users")
    .withIndex("by_tokenIdentifier", (q) => q.eq("tokenIndetifier", tokenIdentifier))
    .first();
...
export const createUser = internalMutation({
  ...
  async handler(ctx, args) {
    await ctx.db.insert("users", {
      tokenIndetifier: args.tokenIdentifier,
      orgIds: [],
      name: args.name,
      image: args.image
    });
...
export const updateUser = internalMutation({
  ...
  async handler(ctx, args) {
    const user = await ctx.db
      .query("users")
      .withIndex("by_tokenIdentifier", (q) => q.eq("tokenIndetifier", args.tokenIdentifier))
      .first();
