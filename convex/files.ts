...
async function hasAccessToOrg(ctx: QueryCtx | MutationCtx, tokenIdentifier: string, orgId: string) {
  ...
  const hasAccess = user.orgIds.includes(orgId) || user.tokenIndetifier.includes(orgId);
  return hasAccess;
}
  ...
  async handler(ctx, args) {
    ...
    if (args.favorites) {
      const user = await ctx.db
        .query("users")
        .withIndex("by_tokenIdentifier", (q) => q.eq("tokenIndetifier", identity.tokenIdentifier))
        .first();
      ...
      const favorites = await ctx.db
        .query("favorites")
        .withIndex("by_userId_orgId_fileId", (q) => q.eq("userId", user?._id).eq("orgId", args.orgId))
        .collect();
